...
def load_data(file_path):
    ...
    s = np.array(data['S1']).reshape((-1, 1)) # selection
    y = np.array(data['Y']).reshape((-1, 1)) # income
...
def train_gan(label_generator, sample_generator, label_discriminator, sample_discriminator, obs_dataloader,
    ...
    for epoch in range(epochs):
        ...
        for index, data in enumerate(obs_dataloader):
            ...
            # 배치 크기 정합성 확보
            min_len = min(obs_t_x_y.shape[0], rep_t_x_y.shape[0])
            obs_t_x_y = obs_t_x_y[:min_len]
            rep_t_x_y = rep_t_x_y[:min_len]
...
res_list = []
total_cnt = Config.experiment_num
start_time = time.time()
for i in range(Config.experiment_num):
    print(f"Data Preparation  : {1 + i}/{total_cnt}")
    ...
    print(f"{i + 1}th Train VAE completed : {time.time() - start_time:.2f} sec")
    ...
    print(f"{i+1}th Train GAN completed : {time.time() - start_time:.2f} sec")
    ...
    # Save generated_samples as a DataFrame for visualization
    generated_df = pd.DataFrame(generated_samples,
                                columns=["X1"] + [f"X2_{i}" for i in range(1, Config.confounds_num + 1)] + ["Y"])
    os.makedirs("C2GAM_master/res", exist_ok=True)
    generated_df.to_csv("C2GAM_master/data/D_gen_init.csv", index=False)
    print("Generated samples saved to: C2GAM_master/res/D_gen_init.csv")
    ...
    print(f"{i+1}th Train Estimator completed : {time.time() - start_time:.2f} sec\n")
...
# 최종 결과 저장
# [0] 관측 데이터 (S=1)에 대한 평균 PEHE
# [1] 생성 데이터 (S=0)에 대한 평균 PEHE
# [2] 관측 데이터에 대한 PEHE 표준편차
# [3] 생성 데이터에 대한 PEHE 표준편차
os.makedirs("res", exist_ok=True)
np.savetxt('res/result.txt', np.concatenate((mae, sd), 0))
print(f"Result saved at : C2GAM_master/res/result.txt")
